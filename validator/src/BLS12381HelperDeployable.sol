// SPDX-License-Identifier: GPL-3.0-or-later

pragma solidity ^0.8.17;

contract BLS12381HelperDeployable {

    constructor() {}

    // g1.One
    bytes internal constant _g1One_1 = hex"";

    bytes internal constant _g1One_2 = hex"";

    function Aggregate() public view returns(bytes memory) {
        return _g1Add(_g1One_1, _g1One_2);
    }

    function _g1Add(bytes memory a, bytes memory b) internal view returns(bytes memory c) {
        bytes memory input = new bytes(256);
        assembly {
            mstore(input, 0) // set length = 0 for append
        }
        _append(input, a);
        _append(input, b);
        require(input.length == 256, "_g1Add malformed input");
        bool success;
        c = new bytes(128);
        assembly {
            success := staticcall(gas(), 0x0b, add(input, 32), 256, add(c, 32), 128)
        }
        if (!success) {
            assembly {
                let ptr := mload(0x40)
                let size := returndatasize()
                returndatacopy(ptr, 0, size)
                revert(ptr, size)
            }
        }
    }

    function _g1Mul(bytes memory point, uint256 scalar) internal view returns(bytes memory c) {
        (bytes memory input) = abi.encodePacked(point, scalar); 
        require(input.length == 160, "_g1Mul malformed input");
        bool success;
        c = new bytes(128);
        assembly {
            success := staticcall(sub(gas(), 2000), 11, add(input, 32), 160, add(c, 32), 128)
        }
        if (!success) {
            assembly {
                let ptr := mload(0x40)
                let size := returndatasize()
                returndatacopy(ptr, 0, size)
                revert(ptr, size)
            }
        }
    }

    // Public wrapper for _g1Add to test it directly
    function testG1Add(bytes memory a, bytes memory b) public view returns(bytes memory) {
        return _g1Add(a, b);
    }

    // Public wrapper for _g1Mul to test it directly
    function testG1Mul(bytes memory point, uint256 scalar) public view returns(bytes memory) {
        return _g1Mul(point, scalar);
    }

    // Get the constants for testing
    function getG1One1() public pure returns(bytes memory) {
        return _g1One_1;
    }

    function getG1One2() public pure returns(bytes memory) {
        return _g1One_2;
    }

    // cheaper than bytes concat :)
    function _append(bytes memory dst, bytes memory src) internal view {
      
        assembly {
            // resize

            let priorLength := mload(dst)
            
            mstore(dst, add(priorLength, mload(src)))
        
            // copy    

            pop(
                staticcall(
                  gas(), 4, 
                  add(src, 32), // src data start
                  mload(src), // src length 
                  add(dst, add(32, priorLength)), // dst write ptr
                  mload(dst)
                ) 
            )
        }
    }
}