...
contract AAStarAccountV6 is IAccount, UUPSUpgradeable, Initializable {
     ...
     * - messagePoint: 256 bytes G2 message point
    ...
    function _validateSignature(UserOperation calldata userOp, bytes32 userOpHash)
        ...
        if (useAAStarValidator && address(aaStarValidator) != address(0)) {
            try this._parseAndValidateAAStarSignature(userOp.signature, hash) 
    ...
    function _parseAndValidateAAStarSignature(
        ...
        // Validate using AAStarValidator
        ...
        ) 
    {
        ...
        uint256 messageOffset = blsOffset + 256;
        uint256 aaOffset = messageOffset + 256;
        
        blsSignature = signature[blsOffset:messageOffset];
        messagePoint = signature[messageOffset:aaOffset];
