...
/**
 * Community KMS+DVT node onboarding portal (CC-40).
 *
 * Ported from the aastar-sdk `node-onboarding-portal` (src/App.tsx) — a 6-step
 * wizard that stakes + registers a DVT node entirely in the operator's browser
 * wallet via `@aastar/sdk`. Only the wallet seam is YAAA-adapted (see
 * `lib/sdk/nodeOnboarding`); the flow logic is unchanged ("页面只串线"). Local /
 * HSM key nodes work today; the KMS-TEE key-less path is gated on CC-37 (KMS
 * `/pop`) and surfaces a pending error until that endpoint is live.
 */
          ...
          <section className="panel">
            ...
            {step === 2 && (
              <div>
                ...
                <p>
                  连接节点 operator 的钱包 —— 它是链上 <code>msg.sender</code>，负责质押 +
                  注册。需持有 ETH(gas)与 GToken(质押 ≥30)。
                </p>
                {wallet ? (
                  <ul className="kv">
                    <li>
                ...
                {cfg.nodeKind === "local" ? (
                  <>
                    <p>
                      ...
                      )。请下载保存，写入节点镜像配置。PoP 由 SDK <code>buildDvtPop</code>{" "}
                      本地推导。
                    </p>
                    <p className="note">
                      ...
                      <ul className="kv">
                        <li>
                          ...
                          <span className="mono danger">
                            ...
                            <button
                              className="mini"
                              onClick={() => download("node-bls.key", blsKey)}
                            >
                              下载
                            </button>
                          </span>
                  ...
                  <>
                    <p>
                      KMS-TEE 节点:BLS 私钥在 TEE 内生成并密封，永不出 TEE。填入 KMS
                      侧节点标识(node_id 或 pubkey)，注册时由 KMS <code>/pop</code> 出 PoP。
                    </p>
                    <div className="form">
                      <label>
                    ...
                    <p className="muted">
                      ⏳ KMS <code>/pop</code> 待上线(CC-37 返工 + A 板 TA 重刷)。此路径注册步会在
                      /pop 未就绪时报错而不提交。
                    </p>
                  </>
                )}
            ...
            {step === 4 && (
              <div>
                ...
                {cfg.nodeKind === "kms-tee" && !KMS_TEE_READY ? (
                  <p className="note">
                    ⏳ KMS-TEE key-less 节点注册暂不可用:依赖 KMS <code>/pop</code> 上线(CC-37 返工
                    + A 板 TA 重刷)。请返回「填写配置」改用<b>本地/HSM 私钥节点</b>完成注册。
                  </p>
                ) : (
                  <>
                    <div className="row">
