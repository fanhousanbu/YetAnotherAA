    ...
    <Layout>
      <div className="dvt-onboard">
        <div className="wrap">
            ...
            {step === 4 && (
              <div>
                ...
                {cfg.nodeKind === "kms-tee" && !KMS_TEE_READY ? (
                  <p className="note">⏳ KMS-TEE key-less 节点注册暂不可用:依赖 KMS <code>/pop</code> 上线(CC-37 返工 + A 板 TA 重刷)。请返回「填写配置」改用<b>本地/HSM 私钥节点</b>完成注册。</p>
                ) : (
                  <>
                    <div className="row">
          ...
          <footer><span>底层:<code>@aastar/sdk</code> onboardDvtNode / buildDvtPop / KMS popSigner · 页面仅串线</span></footer>
        </div>
      </div>
    </Layout>
  );
