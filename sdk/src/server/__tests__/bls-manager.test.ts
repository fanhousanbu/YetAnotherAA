...
describe('BLSManager', () => {
  ...
  describe('getAvailableNodes', () => {
    it('returns active nodes that have apiEndpoint and publicKey', async () => {
      mockAxios.get.mockResolvedValueOnce({
        data: {
          peers: [
            { nodeId: 'n1', nodeName: 'N1', apiEndpoint: 'http://n1.com', publicKey: '0xpk1', status: 'active' },
            { nodeId: 'n2', nodeName: 'N2', apiEndpoint: 'http://n2.com', publicKey: '0xpk2', status: 'active' },
          ],
        },
      });
    ...
    it('tries next seed node after a failure', async () => {
      ...
      mockAxios.get
        ...
        .mockResolvedValueOnce({
          data: {
            peers: [{ nodeId: 'n1', apiEndpoint: 'http://n1.com', publicKey: '0xpk1', status: 'active' }],
          },
        });
  ...
  describe('packSignature', () => {
    ...
    it('throws when nodeIds is missing', () => {
      const { nodeIds: _, ...rest } = validData();
      expect(() => manager.packSignature(rest as any)).toThrow('Missing required signature components');
    });

    it('throws when aaSignature is missing', () => {
      const { aaSignature: _, ...rest } = validData();
      expect(() => manager.packSignature(rest as any)).toThrow('Missing required signature components');
    });

    it('throws when messagePointSignature is missing', () => {
      const { messagePointSignature: _, ...rest } = validData();
      expect(() => manager.packSignature(rest as any)).toThrow('Missing required signature components');
    });
