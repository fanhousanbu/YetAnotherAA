...
describe('KmsManager', () => {
  ...
  describe('createKey', () => {
    ...
    it('POSTs to /CreateKey and returns response data', async () => {
      ...
      const m = new KmsManager({ kmsEndpoint: ENDPOINT, kmsEnabled: true, logger: new SilentLogger() });
...
describe('KmsSigner', () => {
  ...
  it('signMessage calls signHash with EIP-191 message hash', async () => {
    ...
    expect(mockAxios.post).toHaveBeenCalledWith(
      `${ENDPOINT}/SignHash`,
      expect.objectContaining({ Address: ADDRESS, Hash: expect.stringMatching(/^0x[0-9a-f]{64}$/i) }),
      expect.any(Object),
    );
  });
