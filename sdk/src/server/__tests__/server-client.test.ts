...
describe('YAAAServerClient', () => {
  ...
  it('should throw on invalid config (missing rpcUrl)', () => {
    expect(() => new YAAAServerClient(makeConfig({ rpcUrl: '' }))).toThrow(
      'rpcUrl is required',
    );
  });
  ...
  it('should throw on invalid config (missing storage)', () => {
    expect(
      () => new YAAAServerClient(makeConfig({ storage: undefined as any })),
    ).toThrow('storage adapter is required');
  });

  it('should throw on invalid config (missing signer)', () => {
    expect(
      () => new YAAAServerClient(makeConfig({ signer: undefined as any })),
